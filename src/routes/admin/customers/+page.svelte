<script>
  ...
  $: filteredCustomers = allCustomers.filter((customer) => {
    ...
    const matchesCawangan =
      !selectedCawangan || customer.cawangan === selectedCawangan;
    const matchesType = !selectedType || customer.type === selectedType;
    const matchesStatus = !selectedStatus || customer.status === selectedStatus;

    const matchesDateRange =
      (!dateFrom || customer.date >= dateFrom) &&
    ...
    >
      <div>
        ...
        <p class="text-gray-600 mt-2">
          Pengurusan data pelanggan Rayhar Travel
        </p>
      </div>
      <div class="flex items-center space-x-3">
        ...
        <div class="text-sm text-gray-500">
          <span class="font-medium">{filteredCustomers.length}</span> daripada
          <span class="font-medium">{allCustomers.length}</span> pelanggan
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="p-6">
      ...
      >
        ...
        <div class="xl:col-span-2">
          <label
            for="search"
            class="block text-sm font-medium text-gray-700 mb-2">Cari</label
          >
          <div class="relative">
            <input
              type="text"
    ...
    <div class="p-6">
      ...
      {#if filteredCustomers.length > 0}
                  ...
                  <tr class="hover:bg-gray-50 transition-all duration-200">
                    <td class="px-6 py-4">
                      <div class="flex items-center space-x-4">
                        <div
                          class="w-10 h-10 rounded-full bg-gradient-to-br from-primary-600 to-[#AC2EAC] flex items-center justify-center shadow-sm"
                        >
                    ...
                    <td class="px-6 py-4">
                      <Badge
                        variant={customer.status === "Confirmed"
                          ? "success"
        ...
        <div class="lg:hidden space-y-4">
          {#each filteredCustomers as customer}
            <div
              class="bg-white border border-gray-100 rounded-xl p-5 hover:shadow-lg hover:border-gray-200 transition-all duration-200"
            >
              <div class="flex items-start space-x-4">
                <div
                  class="w-12 h-12 rounded-full bg-gradient-to-br from-primary-600 to-[#AC2EAC] flex items-center justify-center flex-shrink-0 shadow-sm"
                >
                  <span class="text-white text-sm font-semibold"
                    >{customer.name.charAt(0)}</span
                  >
